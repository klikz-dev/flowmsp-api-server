...
public class pittsvillefi implements MsgParser {
    private static final Logger log = LoggerFactory.getLogger(pittsvillefi.class);
    ...
    private String get_incident(String msg) {
        var p = Pattern.compile(".* PITTSVILLE FD - (\\d{12}).*");
        var m = p.matcher(msg);
    ...
    private int get_units(String msg, ParsedMessage pm) {
        var p = Pattern.compile(".* Unit\\(s\\) responded: ([A-Z,]+).*");
        var m = p.matcher(msg);
